...
float GetWave(vec2 position, float t)
{
    ...
    return amplitude * sin(dot(directionVector, position) * frequency + t * phase);
};

float AddedWaves(vec2 position, float t)
{
    float waves = 0;
    for(int i = 0; i < numberOfWaves; i++)
    {
        waves += GetWave(position, t);
    }
    return waves;
};
...
void main()
{
    ...
    vertexPosition.y = AddedWaves(vec2(vertexPosition.x, vertexPosition.z), time);
